...
export default async function AdminFunil() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-semibold text-verde-escuro">Funil comercial</h1>
        <p className="text-sm text-foreground/60">Clique no card para abrir a oportunidade completa (timeline, visitas, propostas, contrato, venda).</p>
      </div>
        ...
        {colunas.map((etapa) => {
          ...
          return (
            <div key={etapa} className="w-60 shrink-0">
              <div className={`rounded-t-lg px-3 py-2 text-xs font-semibold flex justify-between ${etapa === "perdido" ? "bg-red-100 text-red-900" : etapa === "fechado" || etapa === "pos_venda" ? "bg-verde text-white" : "bg-verde-escuro text-white"}`}>
                {ETAPA_LABEL[etapa]}
                <span className="tabular-nums">{cards.length}</span>
              </div>
              <div className="bg-areia rounded-b-lg p-2 space-y-2 min-h-24">
                {cards.map((o) => {
                  ...
                  return (
                    <Link key={o.id} href={`/admin/oportunidades/${o.id}`}
                      className="block rounded-lg bg-white border border-linha p-3 hover:shadow transition space-y-1">
                      <p className="font-mono text-[10px] text-foreground/50">{o.codigo}</p>
                      <p className="text-sm font-medium leading-snug">{lead?.nome ?? "—"}</p>
                      <p className="text-xs text-foreground/60 leading-snug">{prop?.titulo}</p>
