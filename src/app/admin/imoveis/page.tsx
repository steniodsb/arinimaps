...
export default async function AdminImoveis({ searchParams }: PageProps<"/admin/imoveis">) {
  ...
  return (
    <div className="space-y-5">
      <h1 className="text-2xl font-semibold text-verde-escuro">Imóveis</h1>
      <div className="flex gap-2 text-sm flex-wrap">
        {Object.keys(FILTROS).map((f) => (
          <Link key={f} href={`/admin/imoveis?filtro=${f}`}
            className={`rounded-full px-4 py-1.5 border capitalize ${f === chave ? "bg-verde text-white border-verde" : "border-linha bg-white hover:bg-areia"}`}>
            ...
            <tbody className="divide-y divide-linha">
              {imoveis.map((p) => (
                <tr key={p.id} className="hover:bg-areia/50">
                  ...
                  <td className="px-4 py-3 font-medium">{p.titulo}<span className="ml-2 text-xs text-foreground/50 capitalize">({p.tipo})</span></td>
