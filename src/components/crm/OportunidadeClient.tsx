...
export default function OportunidadeClient(props: Props) {
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-2">
      {erro && <p className="lg:col-span-2 text-sm text-red-700 bg-red-50 rounded-lg px-4 py-2">{erro}</p>}

      {/* ---------- etapa + encaminhamento ---------- */}
      <section className="rounded-xl border border-linha bg-white p-5 space-y-3">
        <h2 className="font-semibold text-verde-escuro">Etapa do funil</h2>
        <div className="flex gap-2 flex-wrap">
          ...
          <button disabled={ocupado} className={`${btn} bg-red-50 text-red-700 border border-red-200 hover:bg-red-100`}
            onClick={() => {
        ...
        {o.motivo_perda && <p className="text-xs text-red-700">Perdido: {o.motivo_perda}</p>}

        {modo === "arini" && (
          <div className="pt-2 border-t border-linha space-y-2">
            <h3 className="text-sm font-semibold text-verde-escuro">Encaminhamento (intermediação Arini)</h3>
      ...
      {/* ---------- visitas ---------- */}
      <section className="rounded-xl border border-linha bg-white p-5 space-y-3">
        <h2 className="font-semibold text-verde-escuro">Visitas</h2>
        ...
        <div className="space-y-2">
          {props.visitas.map((v) => (
            <div key={v.id} className="rounded-lg bg-areia px-3 py-2 text-sm flex flex-wrap items-center gap-2">
                ...
                <span className="flex gap-1">
                  {(["realizada", "remarcada", "nao_compareceu"] as const).map((s) => (
                    <button key={s} disabled={ocupado}
                      className="text-xs rounded bg-white border border-linha px-2 py-1 hover:bg-verde hover:text-white"
                      onClick={() => {
          ...
          {!props.visitas.length && <p className="text-xs text-foreground/50">Nenhuma visita ainda.</p>}
        </div>
      </section>

      {/* ---------- propostas ---------- */}
      <section className="rounded-xl border border-linha bg-white p-5 space-y-3">
        <h2 className="font-semibold text-verde-escuro">Propostas e contrapropostas</h2>
        <div className="grid gap-2 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          {props.propostas.map((p) => (
            <div key={p.id} className="rounded-lg bg-areia px-3 py-2 text-sm space-y-1">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className={`text-xs rounded-full px-2 py-0.5 ${p.status === "aceita" ? "bg-verde text-white" : p.status === "recusada" ? "bg-red-100 text-red-800" : "bg-white border border-linha"}`}>{p.status}</span>
                {p.status === "enviada" && (
                  <span className="flex gap-1">
              ...
              {p.condicoes && <p className="text-xs text-foreground/60">{p.condicoes}</p>}
            </div>
          ))}
          {!props.propostas.length && <p className="text-xs text-foreground/50">Nenhuma proposta registrada.</p>}
        </div>
      </section>

      {/* ---------- contrato + venda (só Arini) ---------- */}
      {modo === "arini" && (
        <section className="rounded-xl border border-linha bg-white p-5 space-y-4">
          <div className="space-y-2">
            <h2 className="font-semibold text-verde-escuro">Contrato</h2>
            <div className="flex flex-wrap gap-2 items-center text-sm">
              ...
              {(["em_elaboracao", "assinado", "registrado"] as const).map((s) => (
                <button key={s} disabled={ocupado}
                  className="text-xs rounded bg-areia border border-linha px-2 py-1 hover:bg-verde hover:text-white"
            ...
            <div className="flex flex-wrap gap-2 items-center">
              <label className="text-xs rounded-lg border border-linha px-3 py-1.5 cursor-pointer hover:bg-areia">
                Anexar documento
          ...
          <div className="pt-3 border-t border-linha space-y-2">
            <h2 className="font-semibold text-verde-escuro">Registrar venda</h2>
            {props.venda ? (
              ...
              <>
                ...
                <button disabled={ocupado || !venda.valor_final}
                  className={`${btn} bg-ouro text-verde-escuro font-semibold hover:bg-ouro-escuro hover:text-white`}
                  onClick={() => {
      ...
      {/* ---------- timeline ---------- */}
      <section className="rounded-xl border border-linha bg-white p-5 space-y-3 lg:col-span-2">
        <h2 className="font-semibold text-verde-escuro">Timeline do atendimento</h2>
        <div className="flex gap-2">
        ...
        <ol className="space-y-1.5 text-sm">
          {props.eventos.map((e) => (
            <li key={e.id} className="flex gap-3">
              <span className="text-xs text-foreground/40 tabular-nums shrink-0 w-32">
                {new Date(e.created_at).toLocaleString("pt-BR", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}
              </span>
              <span className="flex-1">{e.descricao}{e.autor ? <span className="text-foreground/40"> — {e.autor}</span> : null}</span>
            </li>
          ))}
          {!props.eventos.length && <p className="text-xs text-foreground/50">Sem eventos ainda.</p>}
        </ol>
      </section>
    </div>
  );
